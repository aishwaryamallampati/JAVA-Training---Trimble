...
public final class PDStreamUtils {
	...
	public static void write(final PDPageContentStream stream, final String text, final PDFont font,
			final float fontSize, final float x, final float y, final PDColor color) {
		try {
			stream.beginText();
	...
	public static void rect(final PDPageContentStream stream, final float x, final float y, final float width,
			final float height, final PDColor color) {
		try {
			stream.setNonStrokingColor(color);
			...
			stream.setNonStrokingColor(ColorUtils.getColor(Color.BLACK));
		} catch (final IOException e) {
	...
	public static void rectFontMetrics(final PDPageContentStream stream, final float x, final float y, final PDFont font, final float fontSize) {
		// height
		PDStreamUtils.rect(stream, x, y, 3, FontUtils.getHeight(font, fontSize), ColorUtils.getColor(Color.BLUE));
		// ascent
		PDStreamUtils.rect(stream, x+3, y, 3, FontUtils.getAscent(font, fontSize), ColorUtils.getColor(Color.CYAN));
		// descent
		PDStreamUtils.rect(stream, x+3, y - FontUtils.getHeight(font, fontSize), 3, FontUtils.getDescent(font, 14), ColorUtils.getColor(Color.GREEN));
	}
}
